...
1- The Output will be Null which indicate that there is no second highest salary and it is the case which the table contains only one salary, and I solved this case by counting the total number of distinct salaries in employee table and if there is only one row then it is null
